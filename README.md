...
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:020817,20:061528,55:0c2a50,85:035f8a,100:00b4d8&height=280&section=header&text=Multi-Elevator%20Dispatch&fontSize=52&fontColor=e0f7ff&fontAlignY=38&fontStyle=bold&desc=CTDE%20%2B%20QR-DQN%20%E2%80%94%20Teaching%20three%20elevators%20to%20think%20together%2C%20act%20alone&descAlignY=58&descSize=17&descColor=7dd3fc&animation=fadeIn" width="100%"/>
...
[![Python](https://img.shields.io/badge/Python-3.12-0d1f38?style=for-the-badge&logo=python&logoColor=7dd3fc&labelColor=071020&color=0d1f38)](https://python.org)&nbsp;
[![PyTorch](https://img.shields.io/badge/PyTorch-2.9.0+cu126-0d1f38?style=for-the-badge&logo=pytorch&logoColor=fb923c&labelColor=071020&color=0d1f38)](https://pytorch.org)&nbsp;
[![CUDA](https://img.shields.io/badge/CUDA-Tesla%20T4-0d1f38?style=for-the-badge&logo=nvidia&logoColor=4ade80&labelColor=071020&color=0d1f38)](https://developer.nvidia.com/cuda-zone)&nbsp;
[![Notebook](https://img.shields.io/badge/Notebook-Open%20on%20GitHub-0d1f38?style=for-the-badge&logo=github&logoColor=e2e8f0&labelColor=071020&color=0d1f38)](https://github.com/kumarpiyushraj/multi-elevator-ctde-qrdqn-dispatch/blob/main/Multi_Elevator_CTDE.ipynb)&nbsp;
[![Stars](https://img.shields.io/github/stars/kumarpiyushraj/multi-elevator-ctde-qrdqn-dispatch?style=for-the-badge&logo=github&color=fbbf24&labelColor=0d1117&label=Stars)](https://github.com/kumarpiyushraj/multi-elevator-ctde-qrdqn-dispatch)
...
| 05 | [🔁 Training Sequence & Flow](#-training-sequence-and-decision-flow) | 13 | [📁 Project Structure](#-project-structure) |
...
Three foundational ideas are combined into one unified system:
...
| Metric | 🥇 Ours | ETA-Dispatch | Delta | Verdict |
|:---|:---:|:---:|:---:|:---:|
| Avg Wait Time | **`0.492`** | 0.673 | **−27.0%** | ✅ WIN |
| Energy Consumption | **`427.2`** | 472.7 | **−9.6%** | ✅ WIN |
| Cluster Rate | **`10.0%`** | 10.2% | **−1.6 pp** | ✅ WIN |
| Spread Index | **`0.306`** | 0.297 | **+3.0%** | ✅ WIN |
...
<img src="https://capsule-render.vercel.app/api?type=rect&color=0:003344,100:00080d&height=64&text=%F0%9F%8F%97%EF%B8%8F%20%20Environment&fontSize=22&fontColor=67e8f9&fontAlignY=52&fontAlign=50" width="100%"/>
...
| Training episodes | **3,500** | + 100 warm-up episodes before gradients |
...
> **Reward formula:**
> ```
> reward_i = +8.0×served − 3.0×resp_norm − 0.5×move_cost + 0.3×idle − 2.5×cluster
> ```
> Clipped to `[−20, +20]`
>
...
| Ep 2,500 – 3,500 | `2e-5` | Final polish *(new in v10.3)* |
...
> `score = 1.60` → equal to ETA on all metrics &nbsp;·&nbsp; **Best checkpoint: ep 2,725 → score `1.300`** → wait `0.50`, energy `427.6`, cluster `10.0%`
...
| Ep 1,500 – 1,999 | Every 25 eps | 40 |
| Ep 2,000 – 3,500 | Every 25 eps | 50 |
...
> Avg wait per eval episode vs all 4 baselines across 3,500 training episodes. Vertical dotted lines mark the four LR step-downs. The smoothed RL curve first crosses below ETA-Dispatch (0.673) around **episode 150** and continues to improve through every LR stage, eventually settling ~27% lower.

<div align="center">

<sub>📌 &nbsp;**Figure 1 — Training Convergence** &nbsp;|&nbsp; QR-DQN CTDE vs all baseline policies &nbsp;|&nbsp; lower = better</sub>
...
> 📍 **Best composite checkpoint: ep 2,725** → avg wait `0.50` · energy `427.6` · cluster `10.0%` · composite score `1.300`
...
> The composite score tracks joint quality across wait, energy, and cluster rate simultaneously throughout training — the metric used to select the saved checkpoint. A score of **1.60** means all three metrics are exactly at ETA level; any value below means the RL policy is beating ETA on the weighted combination.

<div align="center">

<sub>📌 &nbsp;**Figure 2 — Composite Checkpoint Score** &nbsp;|&nbsp; lower = better overall policy &nbsp;|&nbsp; reference line at 1.60 = equal to ETA</sub>

<img src="https://github.com/kumarpiyushraj/multi-elevator-ctde-qrdqn-dispatch/blob/main/Results_Diagrams/Composite_Checkpoint.png?raw=true" alt="Composite Checkpoint Score" width="100%"/>

</div>

> 📍 Best ep 2,725 achieves score **1.300** — sitting **19% below** the ETA reference line, confirming joint improvement across all three components simultaneously.

<br/>

---

### 🏆 Figure 3 — Average Wait Time Comparison *(Primary Result)*

> Head-to-head wait-time ranking of all five methods after **60 evaluation episodes** on the restored best checkpoint. This is the single most important metric — it directly measures passenger experience.

<div align="center">

<sub>📌 &nbsp;**Figure 3 — Average Passenger Wait Time Across All Methods** &nbsp;|&nbsp; passengers per floor per step &nbsp;|&nbsp; lower = better</sub>

<img src="https://github.com/kumarpiyushraj/multi-elevator-ctde-qrdqn-dispatch/blob/main/Results_Diagrams/Avg_Wait_Time.png?raw=true" alt="Average Wait Time" width="100%"/>

</div>

> 📍 QR-DQN CTDE: **0.492** — **27.0% below ETA-Dispatch** (0.673) and **94.9% below Nearest Car** (9.679).
...
> Total motor-steps per episode for all five methods. Beating ETA on energy *while simultaneously beating it on wait* is the core challenge — moving less normally means worse wait, and vice versa. This figure confirms the RL agent breaks that tradeoff.

<div align="center">

<sub>📌 &nbsp;**Figure 4 — Energy Consumption Across All Methods** &nbsp;|&nbsp; total motor-steps per episode &nbsp;|&nbsp; lower = better</sub>

<img src="https://github.com/kumarpiyushraj/multi-elevator-ctde-qrdqn-dispatch/blob/main/Results_Diagrams/Energy_Consumption.png?raw=true" alt="Energy Consumption" width="100%"/>

</div>

> 📍 **427.2** motor-steps vs ETA's **472.7** — **9.6% fewer movements**, while also achieving 27% lower wait. SCAN/LOOK is worst at 899.6 (nearly 2× ETA).

<br/>

---

### 🔬 Figure 5 — Training Dynamics *(Three-Panel)*

> The three core metrics evolving **in parallel** across all 3,500 training episodes — wait time (top panel), energy consumption (middle), and elevator cluster rate (bottom). Reading all three together reveals how the policy navigated the three-way tradeoff: it had to reduce wait, reduce energy, *and* reduce clustering simultaneously.

<div align="center">

<sub>📌 &nbsp;**Figure 5 — Training Dynamics: Wait · Energy · Cluster Rate** &nbsp;|&nbsp; all 3,500 episodes &nbsp;|&nbsp; dashed = ETA-Dispatch baseline</sub>
...
> 📍 **Wait** `0.78 → 0.49` &nbsp;·&nbsp; **Energy** `416 → 427` &nbsp;·&nbsp; **Cluster** `52% → 10%` — all three metrics converge toward or below the ETA baseline across 3,500 episodes.

<br/>

---

### 🗺️ Figure 6 — Elevator Spatial Distribution *(Cluster Rate & Spread Index)*

> Cluster rate (left axis) and floor spread index (right axis) over training — the two metrics that capture *how well the elevators self-organise spatially*. A low cluster rate means elevators avoid bunching; a high spread means they distribute across all floors. Both emerge purely from the reward signal — no hard-coded positioning rules.

<div align="center">

<sub>📌 &nbsp;**Figure 6 — Cluster Rate and Spread Index During Training** &nbsp;|&nbsp; dashed = ETA-Dispatch baseline</sub>

<img src="https://github.com/kumarpiyushraj/multi-elevator-ctde-qrdqn-dispatch/blob/main/Results_Diagrams/Elevator_Spatial_Distribution.png?raw=true" alt="Elevator Spatial Distribution" width="100%"/>

</div>

> 📍 Cluster rate falls from ~53% → **10.0%** (matching ETA's 10.2%). Spread index rises from near-zero → **0.306**, exceeding ETA (0.297). Pure emergent self-organisation from reward alone.

<br/>

---

### 🌡️ Figure 7 — Out-of-Distribution Stress Tests

> The trained policy tested on **arrival rates never seen during training** (training cap: λ = 0.35). Throughput bars and wait line show how gracefully the policy degrades as load increases far beyond its training distribution.

<div align="center">

<sub>📌 &nbsp;**Figure 7 — OOD Stress Test: Throughput and Cluster Rate under Unseen Load Levels** &nbsp;|&nbsp; trained on λ ≤ 0.35</sub>

<img src="https://github.com/kumarpiyushraj/multi-elevator-ctde-qrdqn-dispatch/blob/main/Results_Diagrams/Stress_Tests.png?raw=true" alt="OOD Stress Tests" width="100%"/>

</div>

> 📍 At **λ = 0.85** (2.4× the training max): wait rises to 2.02 but spread holds at 0.266 and cluster stays at 20% — structured coverage maintained with no catastrophic collapse.
...
> QR-DQN actor loss across all 3,500 training episodes. Stable monotonic descent with no divergence confirms the four-stage LR schedule and gradient clipping (1.5) worked as intended. Each LR step-down produces a brief expected shift before resuming the descent.

<div align="center">

<sub>📌 &nbsp;**Figure 8 — Actor Network Training Loss** &nbsp;|&nbsp; smoothed curve &nbsp;|&nbsp; dotted verticals = LR step-downs</sub>

<img src="https://github.com/kumarpiyushraj/multi-elevator-ctde-qrdqn-dispatch/blob/main/Results_Diagrams/Actor_Loss.png?raw=true" alt="Actor Loss Trajectory" width="100%"/>

</div>

> 📍 Loss travels **~1.10 → ~0.51** across 3,500 episodes with zero divergence events despite 1,050,000 gradient updates.
...
**Fix:** 500 additional fine-tuning episodes at `LR = 2e-5` (ep 2,500–3,500). Gives the jointly-efficient behaviour at ep 2,200–2,400 time to consolidate.
...
**Fix:** `eval_every=25` after ep 1,500, with 50 eval episodes per checkpoint after ep 2,000 (vs 40 earlier). Dense late-training measurement where quality is highest.
...
<img src="https://capsule-render.vercel.app/api?type=rect&color=0:0f2a1a,100:020a07&height=64&text=%F0%9F%93%9E%20%20Contact%20and%20Support&fontSize=22&fontColor=dcfce7&fontAlignY=52&fontAlign=50" width="100%"/>
